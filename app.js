...
function createInput(layer, x, y, initialText = '') {
    console.log("Creating Input with Font:", state.currentFont, "Color:", state.currentColor);
    ...
    // Explicit Font Styling (Force inline to avoid CSS issues)
    let fontSize = '16px';
    ...
    if (state.currentFont === 'caveat') {
        ...
        fontSize = '32px'; /* Make it bigger */
    }

    content.style.fontSize = fontSize;
    content.style.fontFamily = fontFamily;
    content.classList.add(state.currentFont); // Keep class for reference

    content.contentEditable = true;
    content.innerText = initialText;
    content.style.color = state.currentColor;
    content.dataset.color = state.currentColor;
    content.dataset.font = state.currentFont; // Store font type
    ...
    // Direct click handler for reliability
    delBtn.onclick = (e) => {
        e.preventDefault();
        e.stopPropagation();
...
async function handleDownload() {
    ...
    // For signature, we'll use TimesRomanItalic as a safe fallback for "formal" look 
    // or try URL fetch if we were online. For offline stability, StandardFonts is safest.
    // Let's use Helvetica-BoldOblique or Times-Italic. 
    ...
    inputElements.forEach(el => {
        ...
        const fontSize = fontType === 'signature' ? 24 : 16;

        // ... coords ...

        page.drawText(text, {
            x: pdfX,
            y: pdfY, // Y might need adjustment for font baseline difference
